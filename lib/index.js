...
async function run() {
  ...
  try {
    ...
    */ 
    ...
    const skipPr = (core.getInput('skip_pr') === 'true');
      ...
      try {
        ...
        if (!ignoredRepositories.includes(repo.name)) {        
          ...
          for (const branch of branchesToOperateOn[0]) {
            ...
            /*
             * 4db. Creating new branch in cloned repo, unless the action is configured to
             *      commit straight to the branch it operates on
             */
            ...
            //with skip_pr there is no bot branch to push to, changes land on the branch that is already checked out
            const branchToPushTo = skipPr ? branchName : newBranchName;
            ...
            if (await areFilesChanged(git)) {
              /*
               * 4ed. Pushing files to custom branch, or to the branch itself when PR creation is skipped
               */
              await push(branchToPushTo, commitMessage, committerUsername, committerEmail, git);

              if (skipPr) {
                core.endGroup();
                core.info(`Workflow finished with success and changes for ${repo.name} were pushed directly to ${branchToPushTo} branch`);
              } else {
                /*
              ...
              core.endGroup();
              core.info('Finished with success. No changes were detected');
            }
          }
        }
      } catch (error) {
