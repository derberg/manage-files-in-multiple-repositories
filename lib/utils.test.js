const { access, mkdir, mkdtemp, writeFile } = require('fs').promises;
const { tmpdir } = require('os');
const path = require('path');
const { filterOutMissingBranches, filterOutFiles, getFilteredFilesList, getFileName, getFiles, cleanupClone } = require('./utils');

describe('cleanupClone', () => {
  it('should recursively remove a temporary clone', async () => {
    const cloneDir = await mkdtemp(path.join(tmpdir(), 'manage-files-clone-'));
    const nestedDir = path.join(cloneDir, '.git', 'refs', 'heads');
    await mkdir(nestedDir, { recursive: true });
    await writeFile(path.join(nestedDir, 'main'), 'ref: some-commit-sha');
    await writeFile(path.join(cloneDir, 'file.txt'), 'temporary content');

    await cleanupClone(cloneDir, 'test-repo');

    await expect(access(cloneDir)).rejects.toMatchObject({ code: 'ENOENT' });
  });

  it('should ignore a clone that was not created', async () => {
    await expect(cleanupClone()).resolves.toBeUndefined();
  });

  it('should resolve when the clone directory is already gone', async () => {
    const missingDir = path.join(tmpdir(), 'manage-files-clone-never-created');

    await expect(access(missingDir)).rejects.toMatchObject({ code: 'ENOENT' });

    await expect(cleanupClone(missingDir, 'missing-repo')).resolves.toBeUndefined();
  });

  it('should warn and resolve when removal fails, so other repos keep going', async () => {
    jest.resetModules();
    jest.doMock('fs-extra', () => ({
      copy: jest.fn(),
      remove: jest.fn().mockRejectedValue(new Error('EBUSY: resource busy or locked'))
    }));

    const freshCore = require('@actions/core');
    const warningSpy = jest.spyOn(freshCore, 'warning').mockImplementation(() => {});
    const { cleanupClone: cleanupCloneWithFailingRemove } = require('./utils');

    await expect(cleanupCloneWithFailingRemove('/tmp/some-clone', 'locked-repo')).resolves.toBeUndefined();

    expect(warningSpy).toHaveBeenCalledTimes(1);
    expect(warningSpy.mock.calls[0][0]).toContain('locked-repo');
    expect(warningSpy.mock.calls[0][0]).toContain('EBUSY');
