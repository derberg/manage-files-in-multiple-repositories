const { filterOutMissingBranches, filterOutFiles, getFilteredFilesList, getFileName, getFiles, getAuthanticatedUrl } = require('./utils');

describe('getAuthanticatedUrl', () => {
  it('should put the token in the password position so app installation tokens work', async () => {
    const token = 'ghs_exampleinstallationtoken';
    const url = 'https://github.com/lukasz-lab/.github';

    const expectedUrl = 'https://x-access-token:ghs_exampleinstallationtoken@github.com/lukasz-lab/.github.git';

    const receivedUrl = getAuthanticatedUrl(token, url);
