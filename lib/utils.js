...
function getAuthanticatedUrl(token, url) {
  const arr = url.split('//');
  //the token is passed as the password, with a fixed username, because that is the only
  //form GitHub App installation tokens accept. Personal access tokens work either way,
  //as GitHub ignores the username whenever the password is a valid token
