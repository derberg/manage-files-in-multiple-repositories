...
skip_pr | Boolean value on whether to commit straight to the branch the action operates on, instead of creating a new branch and opening a pull request against it. Useful when the replicated files need no review, as it avoids leaving an open pull request in every single target repository. Note that the push fails for any repository where branch protection forbids pushing directly to that branch, and such a repository is reported as a warning while the action moves on to the next one. When this is enabled, `bot_branch_name` has no effect, as no branch is created. | false | false
